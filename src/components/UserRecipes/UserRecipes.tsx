        ...
        const fetchRecipes = async () => {
            ...
            try {
                const response = await axios.get<Recipe[]>(
                    `https://localhost:7271/api/recipe/GetByUserId/${userId}`
                );
    ...
    const handleOk = async () => {
        ...
        const ingredients = formValues.ingredients.map(
            (ingredient: { name: any; position: any }, index: number) => ({
                id: currentRecipe?.ingredients?.[index]?.id || 0, // Correctly access using a number index
                name: ingredient.name,
                position: ingredient.position,
            })
        );

        // Create instructions with ID preservation for existing ones
        const instructions = formValues.instructions.map(
            (
                instruction: { displayText: string; position: number },
                index: number
            ) => ({
                id: currentRecipe?.instructions?.[index]?.id || 0, // Preserve existing ID or assign 0 for new
        ...
        const recipeData: Recipe = {
            ...
            isPublished: false, // Assuming this is a default or managed separately
            recipeNutrition: {
                id: currentRecipe?.recipeNutrition?.id || 0, // Assuming this is newly created or managed separately
            ...
            timeInfo: {
                id: currentRecipe?.timeInfo?.id || 0, // Assuming this is newly created or managed separately
                ...
                restTime: formValues.timeInfo.restTime.toString() || '', // Optional, provide default if needed
                coolTime: formValues.timeInfo.coolTime.toString() || '', // Optional, provide default if needed
            },
            ingredients: ingredients,
            instructions: instructions,
            searchKeywords: formValues.searchKeywords.map(
                (keyword: { id: any; keyword: any }) => ({
                    id: keyword.id || 0, // Assuming new keywords don't have an ID yet
            ...
            >
                <Form form={form} layout="vertical" name="form_in_modal">
                                ...
                                {fields.map(
                                    ({ key, name, fieldKey, ...restField }) => (
                                        ...
                                        >
                                            ...
                                            >
                                                <Input placeholder="Ingredient Name" className="custom-input"/>
                                            </Form.Item>
                                            <Form.Item
                                                {...restField}
                                                name={[name, 'position']}
                                ...
                                {fields.map(
                                    ({ key, name, fieldKey, ...restField }) => (
                                        ...
                                        >
                                            ...
                                            >
                                                <Input placeholder="Instruction Text" className="custom-input"/>
                                            </Form.Item>
                                            <Form.Item
                                                {...restField}
                                                name={[name, 'position']}
                    ...
                    <div className="recipe-time-info-section">
                        ...
                        >
                            <InputNumber min={1} style={{ width: '100%' }} className="custom-input"/>
                        </Form.Item>
                        ...
                        >
                            <InputNumber min={0} style={{ width: '100%' }} className="custom-input"/>
                        </Form.Item>
