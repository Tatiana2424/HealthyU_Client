...
import { Modal, Input, Form, Spin, message, InputNumber, Space } from "antd";
...
const AdminConfirmeRecipes: React.FC<Props> = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      setIsLoading(true);
      try {
        const recipes = await apiService.get<Recipe[]>(
          "/recipe/GetUnpublishedRecipes"
        );
        setRecipes(recipes);
      } catch (error) {
    ...
    try {
      if (currentRecipe) {
        await apiService.put(
          `/recipe/Publish/${currentRecipe.id}?isPublish=${true}`
        );
              ...
              <>
                ...
                {fields.map(({ key, name, fieldKey, ...restField }) => (
                  ...
                  >
                    ...
                    >
                      <InputNumber placeholder="Position" min={1} disabled />
                    </Form.Item>
                  </Space>
                ))}
              </>
