...
import apiService, { createArticle } from "../../api/apiService";
...
const UserArticles: React.FC<Props> = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      setIsLoading(true);
      try {
        const response = await apiService.get<BlogEntry[]>(
          `/article/GetByUserId/${userId}`
        );
    ...
    if (currentArticle) {
      try {
        await apiService.put(
          `/Article/Update/${currentArticle.id}`,
          articleData
        );
  ...
  const deleteArticle = async (id: string) => {
    try {
      await apiService.delete(`/Article/Delete/${id}`);
