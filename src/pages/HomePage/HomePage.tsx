...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      try {
        const fetchedArticles = await apiService.get<BlogEntry[]>("/Article/GetAll");
  ...
  useEffect(() => {
    const fetchedRecipes = async () => {
      try {
        const fetchedRecipes = await apiService.get<Recipe[]>("/Recipe/GetAllBasePublishedRecipeData");
  ...
  return (
    <div className="home-page">
      ...
      <SearchComponent />
      <div className="spacer" />
      <RecipeList recipes={recipes} onEntryClick={handleEntryRecipeClick} />
      <div className="spacer" />
    </div>
  );
};
