...
const RecipePage: React.FC<RecipePageProps> = ({
    ...
    const fetchedRecipes = async () => {
      try {
        const fetchedRecipes = await apiService.get<Recipe[]>(
          "/Recipe/GetAllBasePublishedRecipeData"
        );
  ...
  useEffect(() => {
    const fetchRecipeById = async (id: any) => {
      try {
        const url = `/Recipe/GetById?id=${id}`;
  ...
  return (
    <div ref={titleRef}>
      ...
      <div className="recipe-page">
        ...
        {recipe?.timeInfo && <CookingTimes timeInfo={recipe?.timeInfo} />}
        ...
        <br />
        <RecipeList
          recipes={recipes.filter((entry) => entry.id.toString() !== id)}
